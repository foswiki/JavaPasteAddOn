...
# =========================
sub handleError {
    my ( $noredirect, $message, $query, $theWeb, $theTopic, $theOopsTemplate,
        $oopsArg1, $oopsArg2 )
      = @_;
    ...
    if ($noredirect) {
        ...
        my $url =
          &TWiki::getOopsUrl( $theWeb, $theTopic, $theOopsTemplate, $oopsArg1,
            $oopsArg2 );
...
sub main {
    ...
    if ( !&TWiki::Store::webExists($webName) ) {
        handleError( $noredirect, "Missing Web", $query, $webName, $topic,
            "oopsnoweb" );
        return;
    }
    ...
    if ($mirrorSiteName) {
        handleError( $noredirect, "This is a readonly mirror",
            $query, $webName, $topic, "oopsmirror", $mirrorSiteName,
            $mirrorViewURL );
        return;
    }
    ...
    if (
        !&TWiki::Access::checkAccessPermission(
            "change", $wikiUserName, "", $topic, $webName
        )
      )
    {
        handleError( $noredirect, "No change permission",
            $query, $webName, $topic, "oopsaccesschange" );
        return;
    }
    ...
    # RNF Added 29 Dec 2001
    my %processedFiles;
    if ( ($archivefile) && ( $fileName =~ /\.zip/ ) ) {
        %processedFiles = doUnzip($tmpFilename);
    }
    else { $processedFiles{$tmpFilename} = "$fileName|$fileComment"; }

    my $fileNameKey;
    my @tmpProc;
    foreach $fileNameKey ( keys %processedFiles )
    {    # RNF Loop through processed files.
        ...
        if ( !$doChangeProperties ) {
            ...
            my $doUnlock        = 1;
            my $dontNotify      = "";
            my $error           = &TWiki::Store::saveAttachment(
                $webName,  $topic,      $text1,
                $saveCmd,  $fileName,   $doNotLogChanges,
                $doUnlock, $dontNotify, $fileComment,
                $tmpFilename
            );

            if ($error) {
                handleError( $noredirect, "Save attachment error",
                    $query, $webName, $topic, "oopssaveerr", $error );
                return;
            }
            ...
            if ($TWiki::doLogTopicUpload) {
                ...
                # write log entry
                &TWiki::Store::writeLog( "upload", "$webName.$topic",
                    $fileName );
    ...
    foreach $fileNameKey ( keys %processedFiles )
    {    # RNF Loop through processed files again.
        @tmpProc = split /\|/, $processedFiles{$fileNameKey};
        $fileName = $tmpProc[0];
        $fileName =~ /^(.*?)$/goi;
        $fileName    = $1;
        ...
        if ($doChangeProperties) {
            TWiki::Attach::updateProperties( $fileName, $hideFile, $fileComment,
                $meta );
        }
        else {
            $fileVersion =
            ...
            my (
                ...
                $tmp7, $tmp9, $mtime, $tmp11, $tmp12, $tmp13
            ) = "";
            (
                $tmp1,  $tmp2,  $tmp3,     $tmp4, $tmp5,
                $tmp6,  $tmp7,  $fileSize, $tmp9, $mtime,
                $tmp11, $tmp12, $tmp13
            ) = stat $fileName;
            $fileDate = $mtime;
            ...
            TWiki::Attach::updateAttachment(
                $fileVersion, $fileName, $filePath,
                $fileSize,    $fileDate, $fileUser,
                $fileComment, $hideFile, $meta
            );
        }

        if ($createLink) {
            my $filePath =
              &TWiki::Store::getFileName( $webName, $topic, $fileName );
            $text =
              addLinkToEndOfTopic( $text, $filePath, $fileName, $fileComment );
        }
    ...
    if ($error) {
        handleError( $noredirect, "Save topic error",
            $query, $webName, $topic, "oopssaveerr", $error );
    }
    else {
...
sub doArchive {
    ...
    if   ($gz) { $isCompressed = 1; }
    else       { $isCompressed = 0; }

    if ( $zip->read($archive) == AZ_OK ) {
        %files = doUnzip( $archive, $web, $topic );
    }
    if ( Archive::Tar->read( $archive, $isCompressed ) ) {
        %files = doUntar( $archive, $isCompressed );
    }
